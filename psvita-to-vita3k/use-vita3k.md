# 使用Vita3K
## 初始设置
1. 选择语言，选择完成后点击右下角继续（一部分中文的游戏需要设置为繁体中文才会显示）；
2. 选择存放路径（即模拟PSVita的文件路径），更改或保持默认路径都可以点击右下角继续（注意的是Android版本在更改路径时需要先授权存储权限）；
...
7. 在Vita3K主页面选择顶部选项栏的`配置`，选择`设置`选项，点击`GUI`勾选`亚洲区域`选项，点击保存并重启Vita3K模拟器以显示正常文字。
...
1. Vita3K不支持任何henkaku及taihen等自制插件，仅固件以及游戏的模块有效；
2. Vita3K不支持读取加密的游戏、DLC、存档等，必须先在Vita3K中安装；
3. Vita3K没有金手指cheat作弊修改的功能，除了使用CE、GG等内存修改器修改；
4. Vita3K没有即时快速存档的功能，想即时存档就只能在游戏中勤存档；
5. 待补充。
