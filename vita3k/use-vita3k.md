...
7. [该步骤仅适用PC，切勿在Android启用]进入到锁屏界面再次点击任何地方进入到Vita3K的Livearea主页面，选择顶部选项栏的`配置`->`设置`选项，点击`GUI`勾选`亚洲区域`选项，点击保存并重启Vita3K模拟器以显示正常文字。
...
- 配置：设置、用户管理
- 控制：虚拟按键（仅Android）、键盘控制（仅PC，Android不存在）、控制器
