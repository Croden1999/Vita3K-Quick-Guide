...
## pkg
1. 打开Vita3K，PC在顶部菜单栏点击`文件`->`安装软件包（pkg）`；Android在主页中点击右下角`+`->`软件包（PKG）`;
2. 选择`pkg`文件，`选择bin/rif文件`或`手动输入zrif密钥`，等待更新补丁安装完成即可。

## zip
因为文件是加密的原因（包含sce_pfs、package），Vita3K存在两种检测work.bin授权文件的方式。

### 存放路径ux0/license内存在授权文件
压缩更新补丁为zip，再到Vita3K上安装zip，等待更新补丁安装完成即可。

### 压缩包sce_sys/package内存在授权文件
